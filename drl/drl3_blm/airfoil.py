...
class airfoil():
    ...
    def create_geometry(self, actions : np.ndarray, name : str, ep : int, plot : bool = False, naca0010 : bool = False):
        ...
        control_points = compute_foil_points(naca0010_foil, foil, ep, self.base_folder, self.path, interp_type="bezier", density = 100, p=6, a=1e-10)
        ...
        # Generate blm mesh
        domain_msh_path = foil.compute_blm_mesh(12, 4, ep)
        try :
            t_file_path = os.path.join(episode_root, "t", "domain.t")
            foil.convert_gmsh_to_mtc(domain_msh_path, t_file_path, False)  # /geometry/mesh/{ep}/t/domain.t
        except Exception as e:
            raise RuntimeError(f"Unable to build domain.t file at episode {ep} : {e}")
        try :
            # Copy to results/.../0/{ep}/cfd/meshes/domain.t
            meshes_dir = os.path.join(self.base_folder, self.path, str(ep), "cfd", "meshes")
            os.makedirs(meshes_dir, exist_ok=True)
            final_dst = os.path.join(meshes_dir, "domain.t")
            shutil.copyfile(t_file_path, final_dst)

        finally :
            if not os.path.isfile(final_dst):
                print(f"WARNING : The final domain.t is not found in {final_dst}", flush=True)
                raise FileNotFoundError(f"Failed to materialize {final_dst}")
        ...
        print(f"Leaving geometry at ep {ep}", flush=True)
