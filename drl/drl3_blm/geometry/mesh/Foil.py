...
class Foil:
    ...
    def compute_blm_mesh(self, length, height, ep: int):
        geo_domain_output = os.path.join(self.geo_dir, f"domain.geo_unrolled")
        msh_domain_output = os.path.join(self.msh_dir, f"domain.msh")
        try:
            ...
            # Add domain points and surface
            p0 = gmsh.model.geo.addPoint(0, 0, 0, 10*self.msh_size)
            p1 = gmsh.model.geo.addPoint(length, 0, 0, 10*self.msh_size)
            p2 = gmsh.model.geo.addPoint(length, height, 0, 10*self.msh_size)
            p3 = gmsh.model.geo.addPoint(0, height, 0, 10*self.msh_size)
            l0 = gmsh.model.geo.addLine(p0, p1)
            l1 = gmsh.model.geo.addLine(p1, p2)
            l2 = gmsh.model.geo.addLine(p2, p3)
            l3 = gmsh.model.geo.addLine(p3, p0)
            gmsh.model.geo.addCurveLoop([l0, l1, l2, l3], 2)

            gmsh.model.geo.addPlaneSurface([2,1])
            gmsh.model.geo.synchronize()
            gmsh.write(geo_domain_output)
            gmsh.option.setNumber("Mesh.MshFileVersion", 2.0)
            
            if os.path.exists(msh_domain_output):
                os.remove(msh_domain_output)
            ...
            if total_elems == 0:
                print(f"ERROR: domain.msh empty mesh (0 elements), ep : {ep}", flush=True)
            gmsh.write(msh_domain_output)
            if (not os.path.isfile(msh_domain_output)) or os.path.getsize(msh_domain_output) == 0:
                print(f"ERROR: domain.msh file missing or empty, ep : {ep}", flush=True)

        except:
            raise RuntimeError(f"gmsh Python API was unable to build the domain around the foil, ep : {ep}.")
        finally:
            gmsh.finalize()
        return msh_domain_output
    ...
    def gmsh4mtc_single_step(self, input, output):
