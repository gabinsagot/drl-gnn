...
class Foil:
    ...
    def __init__(self, number_of_points, chord_length_multiplier, thickness_multiplier):
        ...
        self.origin = self.points[:,:1].argmin() # Furthest left point (often [0.0, 0.0])
    ...
    def create_acp(self, n):
        """
        Args: 
            n (int) : number of intermediate points between each control point
        Returns:
            new_cpoints (np.array of shape (len(self.points)*n, 2)) : np.array containing the intermediate fictive control points
        """
        new_cpoints = []
        points = self.points
        L = len(points)
        ...
        for i in range(L-1):
            ...
            new_cp = np.linspace(a, b, n+2)[1:-1]
            
            new_cpoints.append(new_cp)
        
        
        return np.vstack(new_cpoints)
    
    def create_acp_spline(self, density):
        """
        Args: 
            density (int) : density of intermediate points between each control point (linear)
        Returns:
            new_cpoints (np.array of shape (len(self.points)*n, 2)) : np.array containing the intermediate fictive control points
        """
        new_cpoints = []
    ...
    def get_txt(self):
        ...
        file_path = os.path.join(txt_dir, f"{self.name}.txt")
    ...
    def get_geo(self, 
        ...
        """
        script_dir = os.path.dirname(os.path.abspath(__file__))
        geo_dir = os.path.join(script_dir, "geo_files")
        os.makedirs(geo_dir, exist_ok=True)

        input_file = self.get_txt()
        output_file = os.path.join(geo_dir, f"{self.name}.geo")
    ...
    def get_mesh_windows(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"GMSH FAILED with return code {result.returncode}")
                print(f"STDOUT:\n{result.stdout}")
                print(f"STDERR:\n{result.stderr}")
    ...
    def get_mesh(self):
        ...
        geo_file_path = os.path.join(script_dir, f"geo_files/{self.name}.geo")

        try:
            gmsh.initialize(sys.argv)
            # Ask GMSH to display information in the terminal
            gmsh.option.setNumber("General.Terminal", 1)
            # Open/load the existing .geo file
            gmsh.open(geo_file_path)
            # Set gmsh version 2.0 for output
    ...
    def get_t(self) -> None:
        ...
        script_dir = os.path.dirname(os.path.abspath(__file__))
        t_dir = os.path.join(script_dir, "t_files")
        os.makedirs(t_dir, exist_ok=True)

        input = os.path.join(script_dir, f"msh_files/{self.name}.msh")
        output_filename = self.name + ".t"
        print("t_file to be created...")
        output = os.path.join(t_dir, output_filename)
...
if TEST1:
    ...
    #airfoil.plot()
    #airfoil.apply_transform_point(5, (0.01, -0.01), "translation", 0.05, "max_thickness")
    ncp = airfoil.create_fictive_control_point(n=3)
    print(ncp)
    airfoil.points = np.vstack([airfoil.points, ncp])
    airfoil.order_points()
    airfoil.plot()
