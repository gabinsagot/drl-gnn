...
class Airfoil:
    ...
    def order_point(self):
        ...
        """
        self.points = sorted(self.points, key=lambda p: np.arctan2(p[1], p[0]))

def convert_txt_to_geo(input_file : str,
                       h : float,
                       type : str = "line"
        ) -> str:
    """
    Converts a .txt file to a .geo file for GMSH.
...
def rotate_and_mesh(input_file : str, 
                    angle : float
        ) -> str:
    """
    Rotates and meshes the file 'input_file' by the given 'angle'.

    Args: 
        input_file (str): The path to the input .geo file.
        angle (float): The angle by which to rotate the geometry.

    Returns:
        str: The path to the generated rotated .geo file.
    """
...
def gmsh4mtc_single_step(input : str, 
                         output : str
            ) -> None:
    """
